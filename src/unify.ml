...
type sub = { id:metavar; sub:term; info:node_info }
...
and stream = Sol of sol | Fail of error
and error = Match of node * node | Eos

(* bind_meta: generate a new substitution from a metavariable and a node *)
let bind_meta id n = { id = id; sub = n.term; info = n.pos }

let create_stream g n = Sol { guess = g; next = n }

(* Create an end-of-stream element *)
let create_eos g = create_stream g (fun () -> Fail Eos )
...
let gen_bin_imitation id len ctor n1 n2 =
    ...
    let rec gen_imi n m =
        ...
        | i1, i2 when i1 = len + 1 && i2 = len + 1 -> Fail Eos
...
let rec unify_one (left, right) =
    let error = Fail (Match (left, right)) in
    ...
    | BoundedId id1, BoundedId id2 ->
            if id1 = id2 then create_eos [] else error
    | Id id1, Id id2 -> 
            if id1 = id2 then create_eos [] else error
    | Metavar id, _ ->
...
and unify_head (left, right) tail = function
    | Sol current ->
            ...
            let next_try () = unify_head (left, right) tail next in
            let new_stream = map_stream (fun l -> l @ current.guess) result in
            begin match result, next with
            | Sol _, Sol _ ->
                    (* If it works, try the solution on the tail *)
            ...
            | Sol _, Fail _ ->
                    begin match tail with
                    | hd :: tl ->
                        unify_head hd tl new_stream
                    | [] -> new_stream
                    end
            | Fail _, Sol _ ->
                    (* If the current doesn't work, try the next solution *)
                    unify_head (left, right) tail next
            | Fail f, Fail _ ->
                    (* If it doesn't work, and it was the last solution, return
                     * the error *)
                    Fail f
            end
...
and print_subs = function
    | hd :: tl ->
            hd.id ^ " <- "
            ^ (print_node false { term = hd.sub; pos = Pattern })
            ^ "\n" ^ print_subs tl
    | [] -> ""
...
and print_stream n = function
    | Sol g ->
            ...
            if n < 11 then
                ...
                print_string "more...\n"
    | Fail _ -> print_string "End of solutions\n"
